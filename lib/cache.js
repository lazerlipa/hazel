...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    for (let i = 0; i < matches.length; i += 1) {
      // const nuPKG = url.replace('RELEASES', matches[i]) + '?version=' + this.latest.version.replace('v', '')
      const nuPKG = this.config.url + '/update/win32/' + this.latest.version.replace('v', '') + '/' + matches[i] + '?update=true'
      content = content.replace(matches[i], nuPKG)
    }
    return content
  }

  async cacheNupkg(url) {
    const { token } = this.config
    const headers = { Accept: 'application/octet-stream' }
    ...
    const { status, body } = await retry(
      async () => {
        ...
        if (response.status !== 200) {
          throw new Error(
            `Tried to cache nupkg, but failed fetching ${url}, status ${response.status}`
          )
        }
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      if (name === 'RELEASES') {
        ...
        continue
      } else if (name.includes('.nupkg')) {
        this.latest.files = this.latest.files || {}
        this.latest.files[name] = await this.cacheNupkg(url)
      }
